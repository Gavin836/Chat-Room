...
#This line creates the server’s socket. The first parameter indicates the address 
...
#In this manner, when anyone sends a packet to port 12000 at the IP address of the server 
...
#the client and server can now send bytes to each other over the connection. With TCP, all bytes sent 
#from one side not are not only guaranteed to arrive at the other side but also guaranteed to arrive 
#in order
...
#close the connectionSocket. Note that the serverSocket is still alive waiting for new 
#clients to connect, we are only closing the connectionSocket.
